...
class ActiveSimulator_v0:
    def __init__(self, paths="1", random_seed=42) -> None:
        """
        Initializes the simulator with a network model and sets random seed for reproducibility.
        
        :param paths: Network topology to use ("1" for single edge, "2" for dual edges)
        :param random_seed: Random seed for reproducible experiments (default: 42)
        """
        # Set random seed for reproducibility
        # This affects all subsequent random operations in this process
        random.seed(random_seed)
        
        # Now create the network with the seed already set
        self.network = GroundTruthNetwork(paths)
        self.paths = paths                  # Store the network topology type

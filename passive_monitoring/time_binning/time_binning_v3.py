...
if __name__ == '__main__':
    ...
    print("Starting delay estimation with variable congestion intensity")
    
    # Initialize network and simulator
    network = GroundTruthNetwork(paths="1")
    passive = PassiveSimulator(network)
    
    # Enable variable congestion simulation
    max_intensity = 4.0  # Maximum congestion intensity (higher = more severe congestion)
    seed = 42  # Seed for reproducible congestion patterns
    passive.enable_variable_congestion_simulation(network.DESTINATION, max_intensity=max_intensity, seed=seed)
    ...
    plt.title(f"Delay Distribution with Variable Congestion ({measured_drop_rate:.1%} drop rate, KL={kl_div:.4f})", 
    ...
    congestion_info = (
        ...
        f"Max Congestion Intensity: {max_intensity:.1f}\n"

...
if __name__ == '__main__':
    ...
    # For example, set max_intensity=5.0 and use seed=42 for reproducibility.
    passive.enable_variable_congestion_simulation(network.DESTINATION, max_intensity=5.0, seed=42)
    

    # With variable congestion and extended delays, a slightly coarser bin size is used.
    bin_size = 0.001  # seconds per bin
    ...
    # estimate delays with robust matching and fallback
    # In congested scenarios  fallback (with smoothing) is used to cope with variable drop/delay.
    window_size = 10  
    alpha = 1          
    ...
    delays_ms = [d * 1000 for d in delays]  # convert to ms
    ...
    plt.title("Delay Distribution (Variable Congestion)")
    ...
    # cross correlation and global offset
